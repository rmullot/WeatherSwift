...
extension RawMutex {
    /** RECOMMENDATION: until Swift can inline between modules or at least optimize @noescape closures to the stack,
    if this file is linked into another compilation unit (i.e. linked as part of the CwlUtils.framework
    but used from another module)
    it might be a good idea to copy and paste the relevant `fastsync` implementation
    code into your file (or module and delete `private` if whole module optimization is enabled)
    and use it instead, allowing the function to be inlined.
     ~~~
     ...
     ~~~
     */
    @discardableResult public func sync<R>(execute work: () throws -> R) rethrows -> R {
        unbalancedLock()
...
/// A basic wrapper around `os_unfair_lock`
/// (a non-FIFO, high performance lock that offers safety against priority inversion).
/// This type is a "class" type to prevent accidental copying of the `os_unfair_lock`.
/// - Note: due to the behavior of the lock (non-FIFO) a single thread might drop and reacquire
/// the lock without giving waiting threads a chance to resume (leading to potential starvation of waiters).
/// For this reason, it is only recommended in situations where contention is expected
/// to be rare or the interaction between contenders is otherwise known.
